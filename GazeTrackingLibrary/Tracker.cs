...
namespace GazeTrackingLibrary
{
    public class Tracker
    {
        ...
        private void CalculateGazeCoordinates(TrackData td)
        {
            ...
            String gY = "" + this.gazeDataRaw.GazePositionX;
            String gX = "" + this.gazeDataRaw.GazePositionX;
            gY = gY.Replace(",", ".");
            gX = gX.Replace(",", ".");

            Tracker.dL = this.trackData.PupilDataLeft.Diameter;
            Tracker.dR = this.trackData.PupilDataRight.Diameter;
        ...
        private void appendToFile(string fullpath ,string data)
        {
            FileStream fs = null;
        ...
        private void appendToFileCSV(string fullpath, string data)
        {
            FileStream fs = null;
            if (File.Exists(fullpath))
                fs = new FileStream(fullpath, FileMode.Append, FileAccess.Write);
