...
namespace GazeTrackerUI
{
        ...
        public void changeTextSize()
        {
            while (true)
            {
                try
                {
                    ...
                    double size = GazeTrackerUIMainWindow.FontSize;
                    if (dL > 0 && dR > 0)
                    {
                        //size = size + ((dL + dR) * 0.1);
                        size = ((dL + dR) / 4);
                        GazeTrackerUIMainWindow.QtdEntry++;
                        GazeTrackerUIMainWindow.FontSize = size;
                        GazeTrackerUIMainWindow.Instance.lblTitle.Content = "FontSize1 = " + size;
                    }
                }
                catch (Exception ex)
                {
  ...
  public partial class GazeTrackerUIMainWindow
  {
    ...
    private static GazeTrackerUIMainWindow instance;

    public static double FontSize;
    public static int QtdEntry;
    ...
    public static GazeTrackerUIMainWindow Instance
    {
        get
        {
            if (instance == null)
                instance = new GazeTrackerUIMainWindow();
